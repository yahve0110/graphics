...
const SkillChart = ({ totalSkill }) => {
  ...
  const [highlightedSkill, setHighlightedSkill] = useState(null) // New state to track highlighted skill
    ...
    return colorArr.map((skill, index) => {
      ...
      return (
        <text
          ...
          y={(index + 1) * 5}
          fontSize="3"
          fill={skill.skill === highlightedSkill ? "red" : "black"} // Change text color if highlighted
          onMouseEnter={() => setHighlightedSkill(skill.skill)} // Highlight skill on mouse enter
          onMouseLeave={() => setHighlightedSkill(null)} // Remove highlight on mouse leave
        >
